...
export default function StoryGenres() {
    ...
    // Save button handler to update Firestore document
    const handleSave = async () => {
        const docRef = doc(db, 'storyGenres', 'genres');
        try {
            // Update the 'allGenres' field with the edited genres
            await updateDoc(docRef, {
                allGenres: editGenres
            });
            // Update the state with the new genres
                ...
                <>
                    ...
                    <ul className="list-disc pl-14">
                        {storyGenres.map((genre, index) => (
                            <li key={index} className="text-2xl mt-2">{genre}</li>
                        ))}
                    </ul>
